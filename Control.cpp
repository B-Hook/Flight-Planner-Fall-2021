        ...
        while (!listStack.isEmpty()) {
            ...
            if (!currentPath.isEmpty()) {
                ...
                if (listStack.peek().isCurrNull()) {
                    ...
                    /*if (highestCost < newFlight.getTotalCost())
                        highestCost = newFlight.getTotalCost();
                    if (highestTime < newFlight.getTotalTime())
                        highestTime = newFlight.getTotalTime();
                        */

                    //Itinerary bestFlights;
                    bestFlights.top3(newFlight, sort, highestCost, highestTime);
...
//        Itinerary bestFlights;
//        bestFlights.setTop3(allPossibleFlights, sort, highestCost, highestTime);
//        requestedInfo[count] = bestFlights;

        //itinerary[count] = allPossibleFlights;
        requestedInfo[count] = bestFlights;
        ...
        /*allPossibleFlights.currToFront();
        while (!allPossibleFlights.isCurrNull()) {
            allPossibleFlights.getCurrVal().getPath().currToFront();
            while (!allPossibleFlights.getCurrVal().getPath().isCurrNull()) {
                std::cout << allPossibleFlights.getCurrVal().getPath().getCurrVal().getName() << " (" <<
                            allPossibleFlights.getCurrVal().getPath().getCurrVal().getCarrier() << ") <- ";
                allPossibleFlights.getCurrVal().getPath().currToNext();
            }
            std::cout << departing << " " << allPossibleFlights.getCurrVal().getTotalTime() << " : "
                      << allPossibleFlights.getCurrVal().getTotalCost()<< "\n" << std::endl;
            allPossibleFlights.currToNext();
        }*/
